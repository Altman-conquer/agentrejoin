...
const wrapped = (text: string) => `<agentrejoin-system>\n${text}\n</agentrejoin-system>`;

describe('buildCodexTurnPrompt', () => {
    it('prepends AgentRejoin append system prompt (wrapped) before the first Codex user message', () => {
    ...
    it('does not inject AgentRejoin preamble on normal follow-up turns', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'continue',
            mode: { appendSystemPrompt: APPEND },
    ...
    it('can re-inject AgentRejoin append prompt without title instruction after a thread reset', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'start fresh',
            mode: { appendSystemPrompt: APPEND },
            includeAppendSystemPrompt: true,
