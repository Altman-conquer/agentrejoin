...
function makeSession() {
    return {
        ...
        metadata: {
            ...
            agentRejoinLibDir: '/home/user/.agentrejoin/lib',
            agentRejoinToolsDir: '/home/user/.agentrejoin/tools'
        },
        metadataVersion: 0,
        agentState: null,
        agentStateVersion: 0,
...
describe('ApiSessionClient v3 messages API migration', () => {
    ...
    it('uses an explicit local ID for idempotent session protocol messages', async () => {
        const client = new ApiSessionClient('fake-token', session);
        mockAxiosPost.mockResolvedValueOnce({
            data: {
                messages: [{ id: 'msg-1', seq: 1, localId: 'codex-thread:thread-1:turn-1:0', createdAt: 1, updatedAt: 1 }]
            }
        });
        ...
        client.sendSessionProtocolMessage({
            ...
            role: 'agent',
            turn: 'turn-1',
            ev: { t: 'text', text: 'historical response' }
        }, {
            localId: 'codex-thread:thread-1:turn-1:0'
        });
        ...
        expect(mockAxiosPost.mock.calls[0][1].messages[0].localId).toBe(
            'codex-thread:thread-1:turn-1:0'
        );
    });
    ...
    it('resumes after the captured sequence and routes messages sent during startup', async () => {
        session.seq = 7;
        const client = new ApiSessionClient('fake-token', session);
        const onUserMessage = vi.fn();
        client.onUserMessage(onUserMessage);
        const userMessage = { role: 'user', content: { type: 'text', text: 'sent during resume' } };
        mockAxiosGet.mockResolvedValueOnce({
            data: {
                messages: [{
                    id: 'msg-8',
        ...
        expect(mockAxiosGet.mock.calls[0][1].params.after_seq).toBe(7);
        expect(onUserMessage).toHaveBeenCalledWith(userMessage);
    });
    ...
    it('does not advance the receive cursor from an interleaved outbox response', async () => {
        const client = new ApiSessionClient('fake-token', session);
        (client as any).lastSeq = 10;

        mockAxiosPost.mockResolvedValueOnce({
            data: {
                // seq=11 may belong to a user prompt that arrived while this
                // agent output was being stored as seq=12.
                messages: [{ id: 'msg-12', seq: 12, localId: 'l12', createdAt: 12, updatedAt: 12 }]
            }
        });

        client.sendCodexMessage({ type: 'output' });
        await waitForCheck(() => {
            expect(mockAxiosPost).toHaveBeenCalledTimes(1);
        });
