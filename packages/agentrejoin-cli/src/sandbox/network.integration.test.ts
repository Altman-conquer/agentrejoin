...
const RUN_NETWORK_INTEGRATION = process.env.AGENTREJOIN_RUN_SANDBOX_NETWORK_TESTS === '1';
...
describe('sandbox network integration', () => {
    it.skipIf(!RUN_NETWORK_INTEGRATION)('allows outbound curl in allowed network mode', async () => {
        ...
        const sessionPath = mkdtempSync(join(tmpdir(), 'agentrejoin-sandbox-network-'));
