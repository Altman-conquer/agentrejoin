...
| File | Change |
|------|--------|
| `packages/agentrejoin-app/sources/components/PermissionModeSelector.tsx` | Change both `ModelMode` and `PermissionMode` to `{ key, name, description }` struct |
| `packages/agentrejoin-app/sources/sync/storageTypes.ts` | Change `Session.modelMode` and `Session.permissionMode` to `string | null` (store key only) |
| `packages/agentrejoin-app/sources/sync/storage.ts` | Widen both `updateSessionModelMode()` and `updateSessionPermissionMode()` to accept `string` |
| `packages/agentrejoin-app/sources/components/AgentInput.tsx` | Accept structs for both, render from metadata or hardcoded, show `name` in UI |
| `packages/agentrejoin-app/sources/-session/SessionView.tsx` | Build structs from metadata for both model and mode, pass to AgentInput |
| `packages/agentrejoin-app/sources/sync/sync.ts` | Send model key in meta for all agent types, send permission mode key |
| `packages/agentrejoin-app/sources/sync/typesMessageMeta.ts` | Ensure meta schema accepts any string for permissionMode |
| `packages/agentrejoin-app/sources/app/(app)/new/index.tsx` | Adapt to structs for both model and mode |
...
- ⚠️ `yarn workspace agentrejoin-app lint` is unavailable because the package has no `lint` script
