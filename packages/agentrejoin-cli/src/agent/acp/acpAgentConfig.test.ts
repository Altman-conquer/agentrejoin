...
describe('resolveAcpAgentConfig', () => {
  ...
  it('throws with helpful usage when no args are provided', () => {
    expect(() => resolveAcpAgentConfig([])).toThrow('Usage: agentrejoin acp <agent-name> or agentrejoin acp -- <command> [args]');
  });

  it('throws when separator form omits command', () => {
    expect(() => resolveAcpAgentConfig(['--'])).toThrow('Missing command after "--". Usage: agentrejoin acp -- <command> [args]');
  });
});
