# agentrejoin-agent CLI Tool

## Overview
A new standalone CLI tool (`agentrejoin-agent`) in `packages/agentrejoin-agent` that acts as a dedicated client for controlling Happy Coder agents remotely. Unlike `agentrejoin-cli` which both runs and controls agents, `agentrejoin-agent` only controls them — listing machines, spawning sessions on a machine, creating sessions, sending messages, reading history, monitoring state, and stopping sessions.

This is a completely separate client from `agentrejoin-cli`. It has its own authentication flow (account auth via QR code, same as device linking in the mobile app), its own credential storage (`~/.agentrejoin/agent.key`), and is written from scratch with no code sharing.

## Context
- **Existing system**: Monorepo with `agentrejoin-cli` (agent runtime + control), `agentrejoin-server` (Fastify + PostgreSQL + Redis), `agentrejoin-app` (React Native mobile)
- **Server API**: REST endpoints at `https://api.cluster-fluster.com` + Socket.IO at `/v1/updates`
- **Authentication**: Uses account auth flow (`/v1/auth/account/request` + `/v1/auth/account/response`) — generates ephemeral keypair, displays QR code (`agentrejoin:///account?[base64url-publicKey]`), user scans with existing Happy mobile app to approve, receives encrypted account secret
- **Credential storage**: `~/.agentrejoin/agent.key` (separate from agentrejoin-cli's `~/.agentrejoin/access.key`)
...
- **Integration tests**: Use the real environment bootstrap (`yarn env:up:authenticated`) and exercise the live server + daemon + CLI stack. Do not use mocked acceptance coverage for `agentrejoin-agent spawn`.
...
- [x] Create `packages/agentrejoin-agent/` directory with `package.json` (name: `agentrejoin-agent`, type: module, bin: `./bin/agentrejoin-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/agentrejoin-agent.mjs` entry point wrapper (mirrors agentrejoin-cli pattern: spawns node with `--no-warnings`)
...
- [x] Create `src/config.ts` — reads `AGENTREJOIN_SERVER_URL` (default: `https://api.cluster-fluster.com`), `AGENTREJOIN_HOME_DIR` (default: `~/.happy`), derives credential file path as `${agentRejoinHomeDir}/agent.key`
- [x] Create `src/credentials.ts`:
  ...
  - `readCredentials(config)` — parses `~/.agentrejoin/agent.key` JSON `{ token, secret }`, decodes secret from base64, derives contentKeyPair via `deriveContentKeyPair(secret)`. Returns `Credentials` or `null` if file missing.
  - `writeCredentials(config, token, secret)` — writes `{ token, secret: base64(secret) }` to `~/.agentrejoin/agent.key`
  - `clearCredentials(config)` — deletes `~/.agentrejoin/agent.key`
  - `requireCredentials(config)` — calls `readCredentials`, throws with "Run `agentrejoin-agent auth login` first" if null
...
### Task 4: Authentication command (`agentrejoin-agent auth`)
- [x] Create `src/auth.ts` implementing the account auth flow:
  ...
  3. Generate QR code data: `agentrejoin:///account?` + base64url(keypair.publicKey)
...
- [x] Add `agentrejoin-agent auth login` subcommand that runs the flow above
- [x] Add `agentrejoin-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `agentrejoin-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `agentrejoin-agent`
- [x] `agentrejoin-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `agentrejoin-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `agentrejoin-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `agentrejoin-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`, and supports `--yolo` to send `permissionMode=yolo`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `agentrejoin-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `agentrejoin-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `agentrejoin-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by agentrejoin-agent is visible and controllable from mobile app
- [x] Verify interop: session created by agentrejoin-cli can be listed and history read by agentrejoin-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/agentrejoin-agent/` with usage examples for all commands
- [x] Update root README if it references packages

### Task 12: Machines and spawn
- [x] Add `agentrejoin-agent machines [--active] [--json]`
- [x] Add machine record decryption using the existing account content key derivation and record data key pattern
- [x] Add `agentrejoin-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]`
- [x] Reuse the existing machine RPC contract (`spawn-agentrejoin-session`) without encryption shortcuts
- [x] Add a real integration test that boots `yarn env:up:authenticated`, authenticates `agentrejoin-agent`, lists machines, spawns via the real daemon RPC path, and verifies the live session

## Technical Details

### CLI Commands Summary
```
agentrejoin-agent auth login                          # Authenticate via QR code (scanned by Happy mobile app)
agentrejoin-agent auth logout                         # Clear stored credentials
agentrejoin-agent auth status                         # Show authentication status

agentrejoin-agent machines [--active] [--json]        # List machines
agentrejoin-agent list [--active] [--json]            # List all sessions
agentrejoin-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]  # Spawn on a machine
agentrejoin-agent status <session-id> [--json]        # Get live session state
agentrejoin-agent create --tag <tag> [--path <path>] [--json]  # Create new session
agentrejoin-agent send <session-id> <message> [--yolo] [--wait] [--json]  # Send message
agentrejoin-agent history <session-id> [--limit <n>] [--json]    # Read message history
agentrejoin-agent stop <session-id>                   # Stop a session
agentrejoin-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```

### Authentication Flow (Account Auth)
```
agentrejoin-agent                          Happy Server                    Happy Mobile App
     |                                    |                               |
     ...
     |   agentrejoin:///account?[base64url-key] |                               |
     |                                    |                               |
     |                                    |  <-- User scans QR code ------+
     |                                    |                               |
...
### Credential File Format (`~/.agentrejoin/agent.key`)
```json
{
...
**For new sessions (created by agentrejoin-agent):**
...
**For existing sessions (created by agentrejoin-cli or other clients):**
...
## Post-Completion
**Manual verification:**
- Test full auth flow: run `agentrejoin-agent auth login`, scan QR with Happy app, verify credentials saved
...
- Test `history` command for sessions created by both `agentrejoin-agent` and `agentrejoin-cli`
- Test cross-client interop: messages from agentrejoin-agent readable by mobile app and vice versa

**Distribution:**
- Package can be published to npm as `agentrejoin-agent`
- Alternatively, users install from monorepo via `yarn workspace agentrejoin-agent build`
