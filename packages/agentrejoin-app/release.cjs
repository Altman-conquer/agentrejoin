...
function printAvailableOptions() {
  console.error("Available agentrejoin-app release options:");
...
async function promptForAction() {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.error("Interactive agentrejoin-app release selection requires a TTY.");
  ...
  try {
    ...
    console.error(
      "Run `pnpm install` from repository root, then run `pnpm --filter agentrejoin-app release` again."
    );
    process.exit(1);
  }

  const actionId = await select({
    message: "What should be released for agentrejoin-app?",
    pageSize: 10,
...
async function main() {
  ...
  if (input && !action) {
    console.error(`Unknown agentrejoin-app release option: ${input}`);
    printAvailableOptions();
    process.exit(1);
  }
  ...
  console.log(`Running agentrejoin-app release option: ${action.label}`);
