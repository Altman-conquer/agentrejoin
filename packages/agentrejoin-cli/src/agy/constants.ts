 ...
 * PATH (launchd, or `agentrejoin daemon start` from a non-login shell), so spawning
 * the bare name fails with ENOENT even though agy is installed. Resolution order:
 *   1. `AGENTREJOIN_AGY_PATH` env override — an explicit absolute path to the binary.
...
export function resolveAgyBin(): string {
  const override = process.env.AGENTREJOIN_AGY_PATH;
