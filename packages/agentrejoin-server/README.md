...
AgentRejoin Server is the synchronization backbone for secure Claude Code clients. It enables multiple devices to share encrypted conversations while maintaining complete privacy - the server never sees your messages, only encrypted blobs it cannot read.
...
Your Claude Code clients generate encryption keys locally and use AgentRejoin Server as a secure relay. Messages are end-to-end encrypted before leaving your device. The server's job is simple: store encrypted blobs and sync them between your devices in real-time.

## Hosting

**You don't need to self-host!** The public AgentRejoin relay at `agentrejoin.zhandj.com` uses the same end-to-end encrypted protocol. Encryption happens on your device, and only you hold the keys.

That said, AgentRejoin Server is open source and self-hostable if you prefer running your own infrastructure. The security model is identical whether you use our servers or your own.
...
```bash
docker build -t agentrejoin-server -f Dockerfile .
```
...
docker run -p 3005:3005 \
  -e AGENTREJOIN_MASTER_SECRET=<your-secret> \
  -v agentrejoin-data:/data \
  agentrejoin-server
```
...
Data persists in the `agentrejoin-data` Docker volume across container restarts.

### Environment Variables

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `AGENTREJOIN_MASTER_SECRET` | Yes | - | Master secret for auth/encryption |
...
```bash
# AWS CLI
aws s3api put-bucket-lifecycle-configuration --bucket agentrejoin-blobs \
...
# MinIO
mc ilm rule add myminio/agentrejoin-blobs \
  --expire-days 30 \
  --prefix "sessions/"
```
...
```bash
# AWS CLI
aws s3api put-bucket-encryption --bucket agentrejoin-blobs \
...
# MinIO
mc encrypt set sse-s3 myminio/agentrejoin-blobs
```
