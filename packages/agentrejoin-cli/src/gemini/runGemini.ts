 ...
 * through AgentRejoin CLI. It manages the agent lifecycle, session state, and
 * communication with the AgentRejoin server and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'agentrejoin connect gemini')
  //
  ...
  // (assigned later after AgentRejoin server setup)
  ...
  //
  // Start AgentRejoin MCP server and create Gemini backend
  //

  const happyServer = await startHappyServer(session);
  const bridgeCommand = join(projectPath(), 'bin', 'agentrejoin-mcp.mjs');
  const mcpServers = {
    agentrejoin: {
    ...
    switch (msg.type) {
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: agentrejoin gemini project set <project-id>\n` +
              `Or use a different Google account: agentrejoin connect gemini\n` +
      ...
      case 'tool-result':
        ...
        if (msg.toolName === 'change_title' || 
            ...
            msg.toolName === 'agentrejoin__change_title') {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        pendingChangeTitle = message.message.includes('change_title') || 
                             message.message.includes('agentrejoin__change_title');
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for authentication error (Google Workspace accounts need project ID)
              ...
              errorMsg = `Authentication required. For Google Workspace accounts, you need to set a Google Cloud Project:\n` +
                         `  agentrejoin gemini project set <your-project-id>\n` +
                         `Or use a different Google account: agentrejoin connect gemini\n` +
                         `Guide: https://goo.gle/gemini-cli-auth-docs#workspace-gca`;
            }
