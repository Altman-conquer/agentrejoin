 ...
 *   - AgentRejoin MCP tool usage (`mcp__agentrejoin__change_title`)
 ...
 *   - It does not exercise AgentRejoin's separate local sandbox wrapper; instead it
...
describe.skipIf(!claudeAvailable)('Claude Integration (SDK/query)', { timeout: 180_000 }, () => {
    ...
    it('should clarify natively, resume across a model switch, use TodoWrite, and cross the project boundary with native tools', async () => {
        ...
        const execution = await driver!.runTurn({
            allowedTools: ['mcp__agentrejoin__change_title', 'TodoWrite', 'TodoRead', 'Write', 'Edit', 'Read', 'Glob', 'LS'],
            ...
            prompt: [
                ...
                'Then update the agentrejoin title so it mentions OPTION_B and reply with only DONE.',
            ].join('\n'),
        ...
        expect(execution.toolUseNames).toContain('mcp__agentrejoin__change_title');
    ...
    it('should leave the file untouched and explain the refusal when native write is explicitly disallowed', async () => {
        const denied = await driver!.runTurn({
            allowedTools: ['mcp__agentrejoin__change_title', 'Write', 'Edit', 'Read', 'LS'],
