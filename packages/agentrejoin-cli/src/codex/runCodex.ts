...
type SyncCodexThreadResult =
    ...
    | {
        type: 'busy';
        reason: 'agentrejoin-turn-active' | 'remote-turn-active' | 'sync-in-progress';
    }
    | {
        type: 'error';
        errorMessage: string;
...
export async function runCodex(opts: {
    ...
    try {
        ...
        console.error('  \x1b[36magentrejoin claude\x1b[0m\n');
        process.exit(1);
    }
    ...
    const shouldSyncNewCodexTitle = !reconnectSessionId
        && !opts.resumeThreadId
        && !process.env.AGENTREJOIN_FORK_CODEX_THREAD_ID;
    ...
    const resetCurrentModeDefaults = () => {
        ...
        // a safety guarantee by itself — for plain `agentrejoin codex` the launch
    ...
    // Start AgentRejoin MCP server (HTTP) and prepare STDIO bridge config for Codex
    ...
    const bridgeEntrypoint = join(projectPath(), 'bin', 'agentrejoin-mcp.mjs');
    const mcpServers = {
        agentrejoin: {
        ...
        session.rpcHandlerManager.registerHandler<Record<string, never>, SyncCodexThreadResult>(
            ...
            async () => {
                ...
                if (
                    ...
                    return { type: 'busy', reason: 'agentrejoin-turn-active' };
                }
                ...
                codexThreadSyncInProgress = (async (): Promise<SyncCodexThreadResult> => {
                    try {
                        ...
                        // the imported turns in AgentRejoin.
