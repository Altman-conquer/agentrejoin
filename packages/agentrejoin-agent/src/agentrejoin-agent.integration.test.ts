...
const binPath = resolve(packageDir, 'bin', 'agentrejoin-agent.mjs');
const keepIntegrationEnv = ['1', 'true', 'yes'].includes((process.env.AGENTREJOIN_AGENT_KEEP_ENV ?? '').toLowerCase());
...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'agentrejoin-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'AgentRejoin Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'agentrejoin-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# AgentRejoin Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    ...
    }, 20_000, `session ${sessionId} to appear in agentrejoin-agent list`);
}
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for agentrejoin-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`agentrejoin-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
async function listDaemonSessions(httpPort: number): Promise<Array<{ agentRejoinSessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ agentRejoinSessionId: string; pid: number; startedBy: string }> };
...
describe('agentrejoin-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[agentrejoin-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[agentrejoin-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        const machines = JSON.parse(runAgentCli(['machines', '--json'], agentEnv)) as Array<{
            ...
            metadata?: {
                ...
                resumeSupport?: {
                    ...
                    agentRejoinAgentAuthenticated?: boolean;
                };
            };
        }>;
        ...
        await waitFor(async () => {
            ...
            return refreshedMachine?.metadata?.resumeSupport?.rpcAvailable === true
                && refreshedMachine.metadata.resumeSupport.agentRejoinAgentAuthenticated === true;
        ...
        await waitFor(async () => {
            ...
            return sessions.some(session => session.agentRejoinSessionId === sessionId);
        }, 20_000, 'spawned session to be tracked by daemon');
    });

    it('spawns in the test project root and sends a message through agentrejoin-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'agentrejoin-agent root message';
    ...
    it('spawns in a git worktree and sends a message through agentrejoin-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'agentrejoin-agent worktree message';
        const spawnResult = parseJson<{
            type: 'success' | 'requestToApproveDirectoryCreation' | 'error';
            sessionId?: string;
            machineId?: string;
