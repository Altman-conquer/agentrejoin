...
describe('ensureDaemonRunning', () => {
  ...
  it('returns without spawning when the daemon is already running', async () => {
    ...
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith(
      'Ensuring AgentRejoin background service is running & matches our version...',
    )
  })
  ...
  it('starts the daemon and waits for readiness when the installed version is not running', async () => {
    ...
    vi.stubEnv('AGENTREJOIN_SAFE_ENV', 'kept')
    ...
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith('Starting AgentRejoin background service...')
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith('AgentRejoin background service is ready')
  })
})
