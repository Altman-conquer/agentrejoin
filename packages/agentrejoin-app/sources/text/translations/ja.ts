...
export const ja: TranslationStructure = {
    ...
    errors: {
        ...
        voiceConversationLimitReached: '今月の音声会話の最大数に達しました。将来的にオンデマンドの音声利用を追加する可能性があります。この制限に達した場合は、github.com/nicepkg/agentrejoin/issues で issue を作成してください。',
    ...
    sessionInfo: {
        ...
        agentRejoinSessionIdCopied: 'AgentRejoin Session IDがクリップボードにコピーされました',
        ...
        updateCliInstructions: 'npm install -g agentrejoin@latest を実行してください',
    ...
    machine: {
        ...
        offlineHelp: '• コンピューターがオンラインであることを確認してください\n• `agentrejoin daemon status`を実行して診断してください\n• 最新のCLIバージョンを使用していますか？`npm install -g agentrejoin@latest`でアップグレードしてください',
        daemon: 'デーモン',
        status: 'ステータス',
        stopDaemon: 'デーモンを停止',
