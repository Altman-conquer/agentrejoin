...
/**
 * CLI entry point for agentrejoin command
 * 
...
(async () => {
  ...
  if (!args.includes('--version')) {
    logger.debug('Starting agentrejoin CLI with args: ', process.argv)
  }
  ...
  if (subcommand === 'doctor') {
    ...
    if (args[1] === 'clean') {
      if (args.slice(2).some(a => a === '--help' || a === '-h')) {
        console.log(`
${chalk.bold('agentrejoin doctor clean')} - Kill all agentrejoin-related processes (daemon + sessions)

${chalk.bold('Usage:')}
  agentrejoin doctor clean
...
`)
    ...
    // Handle "agentrejoin gemini model set <model>" command
    ...
    // Handle "agentrejoin gemini model get" command
    ...
    // Handle "agentrejoin gemini project set <project-id>" command
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from AgentRejoin cloud token
    ...
    // Handle "agentrejoin gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  agentrejoin gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "agentrejoin gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: agentrejoin gemini project <command>');
      console.log('');
    ...
    try {
      ...
      console.warn(chalk.yellow('⚠ The gemini backend is deprecated and may be removed in a future release. Use `agentrejoin agy` (Antigravity CLI) instead.'));
    ...
    console.log(chalk.yellow('Note: "agentrejoin logout" is deprecated. Use "agentrejoin auth logout" instead.\n'));
    try {
    ...
    if (daemonSubcommand === 'list') {
      ...
      console.log(`
${chalk.bold('agentrejoin daemon')} - Daemon management
...
${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('agentrejoin doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        showVersion = true
      } else if (arg === '--agentrejoin-starting-mode') {
        ...
        // AgentRejoin-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - AgentRejoin uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by AgentRejoin for session tracking.`))
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('agentrejoin')} - Claude Code On the Go

${chalk.bold('Usage:')}
  agentrejoin [options]         Start Claude with mobile control
  agentrejoin auth              Manage authentication
  ...
  agentrejoin sandbox           Configure and manage OS-level sandboxing
  agentrejoin notify            Send push notification
  agentrejoin daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  agentrejoin doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  agentrejoin                    Start session
  agentrejoin resume cmmij8      Resume a previous session by AgentRejoin session ID
  agentrejoin --yolo             Start with bypassing permissions
                            agentrejoin sugar for --dangerously-skip-permissions
  agentrejoin --chrome           Enable Chrome browser access for this session
  agentrejoin --no-chrome        Disable Chrome even if default is on
  agentrejoin --no-sandbox       Disable the AgentRejoin sandbox for this session
  agentrejoin --js-runtime bun   Use bun instead of node to spawn Claude Code
  agentrejoin --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                           Use a custom API endpoint (e.g., claude-code-router)
  agentrejoin acp gemini         Start Gemini via generic ACP runner
  agentrejoin acp -- opencode --acp
                           Start a custom ACP command
  agentrejoin acp opencode --verbose
                           Print raw ACP backend/envelope events
  agentrejoin auth login --force Authenticate
  agentrejoin doctor             Run diagnostics

${chalk.bold('AgentRejoin supports ALL Claude options!')}
  Use any claude flag with agentrejoin as you would with claude. Our favorite:
...
`)
    ...
    // Show version
    if (showVersion) {
      console.log(`agentrejoin version: ${packageJson.version}`)
      process.exit(0)
    }
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('agentrejoin notify')} - Send notification

${chalk.bold('Usage:')}
  agentrejoin notify -p <message> [-t <title>]    Send notification with custom message and optional title
  agentrejoin notify -h, --help                   Show this help

${chalk.bold('Options:')}
  -p <message>    Notification message (required)
  -t <title>      Notification title (optional, defaults to "AgentRejoin")

${chalk.bold('Examples:')}
  agentrejoin notify -p "Deployment complete!"
  agentrejoin notify -p "System update complete" -t "Server Status"
  agentrejoin notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "agentrejoin notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  if (!credentials) {
    console.error(chalk.red('Error: Not authenticated. Please run "agentrejoin auth login" first.'))
    process.exit(1)
  }
  ...
  try {
    ...
    // Use custom title or default to the product name
    const notificationTitle = title || 'AgentRejoin'
