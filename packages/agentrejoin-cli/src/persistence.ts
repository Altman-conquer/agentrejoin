/**
 * Minimal persistence functions for agentrejoin CLI
 * 
 * Handles settings and private key storage in ~/.agentrejoin/ or local .agentrejoin/
 */
  ...
  try {
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      logger.warn(
        ...
        'Update agentrejoin-cli for full functionality.'
      );
    }
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.agentRejoinHomeDir)) {
    await mkdir(configuration.agentRejoinHomeDir, { recursive: true })
  }
