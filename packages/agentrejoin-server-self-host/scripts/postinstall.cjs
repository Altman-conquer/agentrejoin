 ...
 *
 * prisma/ is a build artifact here — it is copied from packages/agentrejoin-server by
...
if (!fs.existsSync(schema)) {
  console.log('agentrejoin-server-self-host: no prisma/schema.prisma yet, skipping generate (run build first).');
  process.exit(0);
}
