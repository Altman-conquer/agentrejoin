...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const handleCopySessionId = useCallback(async () => {
        ...
        try {
            ...
            Modal.alert(t('common.success'), t('sessionInfo.agentRejoinSessionIdCopied'));
        } catch (error) {
    ...
    const handleCopyUpdateCommand = useCallback(async () => {
        const updateCommand = 'npm install -g agentrejoin@latest';
        try {
    ...
    return (
        <>
            <ItemList>
                ...
                <ItemGroup>
                    <Item
                        title={t('sessionInfo.agentRejoinSessionId')}
                    ...
                    {/* TODO: migrate to `agentrejoin resume <agentrejoin-session-id>` once it works without agentrejoin-agent auth */}
                ...
                {session.metadata && (
                    <ItemGroup title={t('sessionInfo.metadata')}>
                        ...
                        {session.metadata.agentRejoinHomeDir && (
                            <Item
                                title={t('sessionInfo.happyHome')}
                                subtitle={formatPathRelativeToHome(session.metadata.agentRejoinHomeDir, session.metadata.homeDir)}
