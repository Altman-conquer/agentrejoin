 ...
 * Priority: AGENTREJOIN_CLAUDE_PATH env var > PATH > npm > Bun > Homebrew > Native
 * @returns {{path: string, source: string}|null} Path and source, or null if not found
 */
function findGlobalClaudeCliPath() {
    // 1. Environment variable (explicit override)
    const envPath = process.env.AGENTREJOIN_CLAUDE_PATH;
    if (envPath && fs.existsSync(envPath)) {
        const resolved = resolvePathSafe(envPath) || envPath;
        return { path: resolved, source: 'AGENTREJOIN_CLAUDE_PATH' };
    }
...
function formatGoalHookJsonValidationWarning(version) {
    return [
        ...
        'or use AgentRejoin remote mode with the bundled Claude Code runtime.'
    ].join(' ');
}
...
function runClaudeCli(cliPath) {
    ...
    //
    // CRITICAL: when agentrejoin-cli aborts the launcher (terminal→remote switch),
    ...
    child.on('exit', (code, signal) => {
        // Mirror the binary's exit so agentrejoin-cli sees the same status. If the
