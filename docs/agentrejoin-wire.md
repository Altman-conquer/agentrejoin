...
This document describes the shared wire package: `agentrejoin-wire`.

## Why this package exists

Before `agentrejoin-wire`, wire-level message and session-protocol schemas were duplicated across packages (CLI, app, server, and agent). That caused drift risk and made protocol evolution harder.

`agentrejoin-wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `agentrejoin-wire`
- workspace path: `packages/agentrejoin-wire`
...
### CLI (`packages/agentrejoin-cli`)

- Session protocol imports now reference `agentrejoin-wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `agentrejoin-wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `agentrejoin-wire`.

### App (`packages/agentrejoin-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `agentrejoin-wire`:
  - `ApiMessageSchema`
  - `ApiUpdateNewMessageSchema`
  - `ApiUpdateSessionStateSchema`
  - `ApiUpdateMachineStateSchema`

### Server (`packages/agentrejoin-server`)

- Prisma JSON message content type now references `SessionMessageContent` from `agentrejoin-wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/agentrejoin-agent`)

- `RawMessage` now aliases `SessionMessage` from `agentrejoin-wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `agentrejoin-wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`agentrejoin-wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn release
# choose agentrejoin-wire
```

or:

```bash
yarn workspace agentrejoin-wire release
```

When building workspaces from a clean checkout, build `agentrejoin-wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `agentrejoin-wire`.
4. Update downstream package versions if needed.
5. Publish dependent package updates only after the new `agentrejoin-wire` version is available.

## Notes

- `agentrejoin-wire` should stay focused on wire contracts only (types + Zod schemas + small helpers).
