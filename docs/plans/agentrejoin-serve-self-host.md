...
Ship a zero-network, fully self-hosted Happy in the regular `npm i -g happy` package. One foreground command — `happy server` — runs the sync server + web app on `localhost` and writes the local URL into `~/.agentrejoin/settings.json`. Every other Happy process (daemon, `happy claude`, etc.) reads that URL from settings. No analytics, no external endpoints, no extra installs.
...
Add to `Settings` in `packages/agentrejoin-cli/src/persistence.ts:37`:
...
- Users can also just edit `~/.agentrejoin/settings.json` by hand.
...
`packages/agentrejoin-cli/src/configuration.ts:32` becomes:

```ts
this.serverUrl =
  process.env.AGENTREJOIN_SERVER_URL              // env var still wins (debug/override)
  || settings.serverUrl                     // NEW — from ~/.agentrejoin/settings.json
...
```html
<script>window.__AGENTREJOIN_CONFIG__ = { serverUrl: location.origin, disableAnalytics: true };</script>
```

And `packages/agentrejoin-app/sources/sync/serverConfig.ts:10-14` gets one extra fallback:

```ts
export function getServerUrl(): string {
  return serverConfigStorage.getString(SERVER_KEY) ||
         (globalThis as any).__AGENTREJOIN_CONFIG__?.serverUrl ||  // NEW
         process.env.EXPO_PUBLIC_AGENTREJOIN_SERVER_URL ||
         DEFAULT_SERVER_URL;
}
```

Production `app.happy.engineering` never has `__AGENTREJOIN_CONFIG__` set → no behavior change there. Self-host injects it → web calls its own origin. One bundle, two behaviors.
...
- **agentrejoin-cli, agentrejoin-agent, agentrejoin-server: no analytics at all** (verified — see Audit reference below).
- **agentrejoin-app (web + mobile): PostHog only**, already gated: `tracking = config.postHogKey ? new PostHog(...) : null` (`packages/agentrejoin-app/sources/track/tracking.ts:4`). Every call site is `tracking?.capture(...)`, so absence of the key already short-circuits everything.
...
const analyticsDisabled =
  ...
  (globalThis as any).__AGENTREJOIN_CONFIG__?.disableAnalytics === true;
...
- `window.__AGENTREJOIN_CONFIG__.disableAnalytics = true` injected by agentrejoin-server's static handler.
...
No analytics code exists. Nothing to disable. (If we ever add CLI telemetry in the future, the same env var name `AGENTREJOIN_DISABLE_ANALYTICS=1` should be the kill switch — drop `EXPO_PUBLIC_` prefix.)
...
3. **Web app**: `window.__AGENTREJOIN_CONFIG__` only ever set by the static handler in self-host mode. Production builds at `app.happy.engineering` never see it.
...
```
happy@1.x.x/
├── dist/                        existing — bundled JS (now imports @slopus/agentrejoin-server)
├── bin/                         existing
├── tools/                       existing — ripgrep + difft binaries
├── server-assets/               NEW (~30 MB total)
│   ├── webapp/                  pre-built agentrejoin-app web export (~28 MB)
│   │   ├── index.html
│   │   ├── _expo/
│   │   └── assets/
│   ├── pglite/
...
**`packages/agentrejoin-cli/src/commands/server.ts`** (~80 LOC)
```
...
**`packages/agentrejoin-cli/scripts/build-server-assets.mjs`** (~50 LOC, build-time)
- `pnpm --filter @slopus/agentrejoin-app build:web`
- Copy webapp dist, pglite wasm, server migrations → `packages/agentrejoin-cli/server-assets/`

**`packages/agentrejoin-server/sources/index.ts`** (NEW, ~40 LOC)
...
```
When `staticDir` set, register `@fastify/static` on `/*` with an `onSend` hook that prepends the `__AGENTREJOIN_CONFIG__` script tag into `index.html`.

### Modified files

**`packages/agentrejoin-cli/package.json`**
- Dep: `"@slopus/agentrejoin-server": "workspace:*"` (only — fastify-static and pglite come transitively)
- `files`: add `"server-assets"`
- `build`: run `build-server-assets.mjs` before `pkgroll`

**`packages/agentrejoin-cli/src/index.ts`**
- One new subcommand branch for `server` (~10 LOC)

**`packages/agentrejoin-cli/src/persistence.ts`**
- Add `serverUrl?: string` to `Settings` (~2 LOC)

**`packages/agentrejoin-cli/src/configuration.ts`**
- Add `settings.serverUrl` to URL precedence (~3 LOC, but needs sync settings read at bootstrap)

**`packages/agentrejoin-server/package.json`**
- Flip `"private": false`
- Rename to `"@slopus/agentrejoin-server"`
- Add `"main": "./sources/index.ts"` + `exports` map
- Add `@fastify/static` dep

**`packages/agentrejoin-app/sources/sync/serverConfig.ts`**
- 3-line `__AGENTREJOIN_CONFIG__` fallback

**`packages/agentrejoin-app/sources/track/tracking.ts`**
- 3-layer kill switch (env var + `__AGENTREJOIN_CONFIG__.disableAnalytics`) (~8 LOC)
...
| `@slopus/agentrejoin-server/index.ts` exports | ~40 |
| Build script | ~50 |
...
- Generated on first `happy server` start: 32 random bytes → `~/.agentrejoin/server/master-secret` (0600 perms).
...
3. **Sharp dep in agentrejoin-server** — ~20 MB native binary per platform. Either npm optional-deps (sharp does this itself) or stub out image processing for the self-host minimum.
...
2. Export `migrate` / `startServer` from `@slopus/agentrejoin-server`, flip `private: false`, add `@fastify/static`. No functional change anywhere else. Verify package publishes cleanly.
3. Add `window.__AGENTREJOIN_CONFIG__` fallback to `serverConfig.ts`. No behavior change in production.
...
- `PUBLIC_WEBAPP_URL` for agentrejoin-server OAuth redirects.
...
### Analytics
Only `agentrejoin-app` has PostHog (`packages/agentrejoin-app/sources/track/tracking.ts:4`), already gated by `config.postHogKey`. CLI / agent / server have zero analytics (grepped).

### Server URL configurability (already exists)
- CLI / agent: `AGENTREJOIN_SERVER_URL` env var (default `https://api.cluster-fluster.com`).
- Web app: in-app Settings → Server screen + `EXPO_PUBLIC_AGENTREJOIN_SERVER_URL`.
- Server: `pnpm standalone:dev` runs with PGlite — no Docker, no Redis. Entry: `packages/agentrejoin-server/sources/standalone.ts`.

### Hardcoded `happy.engineering` URLs (cosmetic)
- `agentrejoin-app/sources/components/SettingsView.tsx:396` — privacy link
- `agentrejoin-cli/src/ui/doctor.ts:225`, `src/claude/utils/systemPrompt.ts:20-23`, `src/commands/connect.ts:74` — docs / attribution
- `agentrejoin-server/sources/app/api/routes/connectRoutes.ts` — GitHub OAuth redirect
- `agentrejoin-app/app.config.js:46,77` + iOS/Android manifests — universal link domain
