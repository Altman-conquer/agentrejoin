...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from AgentRejoin cloud (via 'agentrejoin connect gemini') - highest priority */
  cloudToken?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. AgentRejoin cloud OAuth token (via 'agentrejoin connect gemini') - highest priority
  ...
  let apiKey = options.cloudToken       // 1. AgentRejoin cloud token (passed from runGemini)
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'agentrejoin connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
