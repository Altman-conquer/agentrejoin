...
export function query(params: { prompt: QueryPrompt; options?: QueryOptions }): Query {
    ...
    // so sessions AgentRejoin starts/continues remain visible to a plain
    // `claude --resume` picker. The agent SDK would otherwise default to
    // CLAUDE_CODE_ENTRYPOINT="sdk-ts" and the picker would hide every AgentRejoin
    // session. See slopus/agentrejoin#1202.
