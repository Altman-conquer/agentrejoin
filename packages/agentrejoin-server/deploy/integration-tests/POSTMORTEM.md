...
## Setup
- 2 agentrejoin-server replicas behind a `LoadBalancer` service via `minikube tunnel`
...
**Observation:** `kubectl exec agentrejoin-redis-0 -- redis-cli XINFO STREAM socket.io`
...
1. **Re-register on every reconnect** (client side, daemon)
    - Already done by agentrejoin-cli for fresh connects but the
