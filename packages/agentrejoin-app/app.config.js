...
function loadBuildMetadata() {
    const commitSha =
        process.env.AGENTREJOIN_BUILD_COMMIT_SHA ||
        process.env.EAS_BUILD_GIT_COMMIT_HASH ||
        process.env.GITHUB_SHA ||
        git(['rev-parse', 'HEAD']);
    const commitTimestamp =
        process.env.AGENTREJOIN_BUILD_COMMIT_TIMESTAMP ||
        (commitSha
...
export default {
    expo: {
        ...
        android: {
            versionCode: Number(process.env.AGENTREJOIN_ANDROID_VERSION_CODE) || 9,
            adaptiveIcon: {
