# AgentRejoin CLI Codebase Overview

## Project Overview

AgentRejoin CLI (`agentrejoin`) is a command-line tool that wraps coding agents to enable remote control and session sharing. It's part of a three-component system:

1. **agentrejoin** (this project) - CLI wrapper for coding agents
2. **agentrejoin-app** - React Native mobile and web client
3. **agentrejoin-server** - Node.js server with Prisma
...
## Starting the Daemon
```bash
# From the agentrejoin-cli directory:
./bin/agentrejoin.mjs daemon start

# With custom server URL (for local development):
AGENTREJOIN_SERVER_URL=http://localhost:3005 ./bin/agentrejoin.mjs daemon start

# Stop the daemon:
./bin/agentrejoin.mjs daemon stop

# Check daemon status:
./bin/agentrejoin.mjs daemon status
```

## Daemon Logs
- Daemon logs are stored in `~/.agentrejoin-dev/logs/` (or `$AGENTREJOIN_HOME_DIR/logs/`)
