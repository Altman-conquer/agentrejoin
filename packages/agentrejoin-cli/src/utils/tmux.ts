 ...
 * Ensures consistent tmux handling across agentrejoin-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "agentrejoin";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "agentrejoin"
            // - empty string: Use first existing session or create "agentrejoin"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "agentrejoin"
                    sessionName = 'agentrejoin';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `agentrejoin-${Date.now()}`;
