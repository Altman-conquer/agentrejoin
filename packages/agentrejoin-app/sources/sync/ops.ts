...
export interface SpawnSessionOptions {
     ...
     * AgentRejoin session attaches to a pre-existing on-disk Claude conversation
     * file. Used by the session fork / duplicate flow.
     */
    resumeClaudeSessionId?: string;
    /**
     * If set, the daemon spawns Codex with `--resume <id>` so the new AgentRejoin
 ...
 * the new Claude session UUID. Caller then spawns a fresh AgentRejoin session
 * with `resumeClaudeSessionId` set to that UUID to attach a new AgentRejoin
 ...
 *   2. Spawn a new AgentRejoin session on the same machine with
 ...
 * RPC into env vars, then into the new AgentRejoin session's metadata at start
