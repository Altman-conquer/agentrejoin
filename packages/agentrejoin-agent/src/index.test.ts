...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, AGENTREJOIN_HOME_DIR: '/tmp/nonexistent-agentrejoin-test' } });
...
describe('agentrejoin-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('agentrejoin-agent');
        expect(stdout).toContain('CLI client for controlling AgentRejoin sessions remotely');
    });
