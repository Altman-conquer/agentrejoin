...
async function makeTempDir(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), 'agentrejoin-codex-image-input-'));
    tempDirs.push(dir);
    return dir;
}
...
describe('resolveCodexImageCacheDir', () => {
    it('uses the explicit cache root when provided', () => {
        expect(resolveCodexImageCacheDir({
            cacheRootDir: '/tmp/agentrejoin-cache',
            sessionId: 'session-1',
        })).toBe('/tmp/agentrejoin-cache/session-1');
    });

    it('defaults to AgentRejoin local state instead of arbitrary OS temp', () => {
        expect(resolveCodexImageCacheDir({
            sessionId: 'session-4',
    ...
    it('keeps malformed session ids inside the cache root', () => {
        const cacheRootDir = '/tmp/agentrejoin-cache';
