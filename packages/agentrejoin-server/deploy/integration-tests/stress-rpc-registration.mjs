...
// 1. Machine daemon connects + registers spawn-agentrejoin-session (fire-and-forget)
// 2. "iOS" caller immediately calls spawn-agentrejoin-session via RPC
...
async function iosSessionFlow() {
    ...
    for (let round = 0; round < ROUNDS; round++) {
        ...
        const MACHINE_METHOD = `${machineId}:spawn-agentrejoin-session`;
