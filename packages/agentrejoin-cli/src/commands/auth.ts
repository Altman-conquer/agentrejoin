...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('agentrejoin auth')} - Authentication management

${chalk.bold('Usage:')}
  agentrejoin auth login [--force]    Authenticate with AgentRejoin
  agentrejoin auth logout             Remove authentication and machine data
  agentrejoin auth status             Show authentication status
  agentrejoin auth help               Show this help message
...
async function handleAuthLogout(): Promise<void> {
  ...
  const happyDir = configuration.agentRejoinHomeDir;
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      console.log(chalk.gray('  Run "agentrejoin auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "agentrejoin auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "agentrejoin auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.agentRejoinHomeDir}`));
