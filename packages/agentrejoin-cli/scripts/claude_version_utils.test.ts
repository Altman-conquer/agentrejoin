...
describe('Claude Version Utils - Cross-Platform Detection', () => {
  ...
  describe('resolveClaudeEntrypoint', () => {
    ...
    beforeEach(() => {
      packageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentrejoin-claude-entrypoint-'));
    });
...
describe('AGENTREJOIN_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.AGENTREJOIN_CLAUDE_PATH;
  });

  it('should use AGENTREJOIN_CLAUDE_PATH when set', () => {
    process.env.AGENTREJOIN_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('AGENTREJOIN_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('AGENTREJOIN_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.AGENTREJOIN_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('AGENTREJOIN_CLAUDE_PATH');
  });
});
