# Minimal Fix Plan for AgentRejoin-CLI Bugs with TDD
# Date: 2025-01-15
...
**Problem**: When running `./bin/agentrejoin.mjs --continue`, Claude CLI returns error:
```
Error: --session-id cannot be used with --continue or --resume
```

**Root Cause Analysis**:
- This is a Claude Code 2.0.64+ design constraint, NOT a agentrejoin-cli bug
- AgentRejoin-CLI generates a NEW session ID and adds `--session-id <uuid>` for all local sessions
...
2. **Remote Mode (No conflict)**:
   ...
   → No --session-id added by agentrejoin-cli
   → Works fine
   ```
...
## Bug 2: AgentRejoin Server Unavailability Crash

**Problem**: AgentRejoin-CLI crashes when AgentRejoin API server is unreachable

**Server Details**:
- Default server: `https://api.cluster-fluster.com`
- Environment variable: `AGENTREJOIN_SERVER_URL` (overrides default)
- Local development: `http://localhost:3005`
- The server handles session management and real-time communication for AgentRejoin CLI
...
try {
    ...
    console.log('⚠️  Cannot connect to AgentRejoin server - continuing in local mode');
...
```typescript
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        console.log('⚠️  AgentRejoin server unreachable - working in offline mode');
...
describe('ApiSessionClient connection handling', () => {
    it('should handle socket connection failure gracefully', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  Cannot connect to AgentRejoin server - continuing in local mode'
        );
...
describe('Api server error handling', () => {
    it('should return null when AgentRejoin server is unreachable', async () => {
        const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  AgentRejoin server unreachable - working in offline mode'
        );
...
5. **Refactor Phase**:
   - Add session ID extraction for --continue (future enhancement):
     ...
     - Update AgentRejoin's session metadata with Claude's ID
   - Ensure code is clean and minimal

6. **Manual Verification**:
   ```bash
   # Test Bug 1 fix:
   ./bin/agentrejoin.mjs --continue  # Should work without error
   # Verify mobile/daemon still work with session ID

   # Test Bug 2 fix:
   AGENTREJOIN_SERVER_URL=http://invalid:9999 ./bin/agentrejoin.mjs  # Should show warning, not crash
...
**Bug 1 Fixed**:
- Test: `./bin/agentrejoin.mjs --continue` exits with code 0
- No "session-id cannot be used" error

**Bug 2 Fixed**:
- Test: `AGENTREJOIN_SERVER_URL=http://invalid:9999 ./bin/agentrejoin.mjs` shows warning message
- Process continues in local mode instead of crashing
- Clear user feedback: "⚠️ AgentRejoin server unreachable - working in offline mode"
