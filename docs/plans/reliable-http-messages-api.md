...
⚠️ No lint script is defined in `packages/agentrejoin-server/package.json`, so linter execution is currently not available.
...
- Update `agentrejoin-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `agentrejoin-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
