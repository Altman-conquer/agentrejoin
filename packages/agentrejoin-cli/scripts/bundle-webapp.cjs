...
/**
 * Runs `expo export -p web` in packages/agentrejoin-app and copies the output into
 * a package-owned artifact directory. By default this writes to agentrejoin-cli/tools/webapp
 * for local development. Release packaging can pass --out-dir to place it elsewhere.
 *
 * agentrejoin-cli does NOT depend on agentrejoin-app — we reach into the sibling at build time only.
 */
...
const APP_DIR = path.resolve(REPO_ROOT, 'packages/agentrejoin-app');
...
function main() {
    ...
    console.log(`→ Building agentrejoin-app web bundle (expo export)`);
    rmrf(APP_DIST);
