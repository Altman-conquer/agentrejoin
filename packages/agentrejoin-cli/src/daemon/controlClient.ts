...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.AGENTREJOIN_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.AGENTREJOIN_DAEMON_HTTP_TIMEOUT) : 10_000;
...
export async function notifyDaemonSessionStarted(
  ...
  // app's resume-agentrejoin-session RPC fails with "not tracked by this daemon".
 ...
 * For instance we are not handling the case where we upgraded agentrejoin,
 ...
 * For instance when running `agentrejoin daemon status` we can show more information.
 */
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  // from the bundled version — e.g. when `agentrejoin-coder@0.13.1` was published as
