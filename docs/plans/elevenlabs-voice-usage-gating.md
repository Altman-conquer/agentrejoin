...
- Server token route: `packages/agentrejoin-server/sources/app/api/routes/voiceRoutes.ts`
- Client token fetch: `packages/agentrejoin-app/sources/sync/apiVoice.ts`
- Voice start decision: `packages/agentrejoin-app/sources/realtime/RealtimeSession.ts`
- ElevenLabs client session start:
  - `packages/agentrejoin-app/sources/realtime/RealtimeVoiceSession.tsx`
  - `packages/agentrejoin-app/sources/realtime/RealtimeVoiceSession.web.tsx`

## Existing Secret Assumptions

The repo already assumes ElevenLabs API access exists on the server:

- `packages/agentrejoin-server/sources/app/api/routes/voiceRoutes.ts` reads `process.env.ELEVENLABS_API_KEY`.
- `packages/agentrejoin-server/deploy/handy.yaml` extracts `/handy-elevenlabs`.
...
Update `packages/agentrejoin-server/sources/app/api/routes/voiceRoutes.ts` to:
...
- `packages/agentrejoin-app/sources/realtime/types.ts`
- `packages/agentrejoin-app/sources/realtime/RealtimeVoiceSession.tsx`
- `packages/agentrejoin-app/sources/realtime/RealtimeVoiceSession.web.tsx`
- `packages/agentrejoin-app/sources/realtime/RealtimeSession.ts`
- `packages/agentrejoin-app/sources/sync/apiVoice.ts`
...
There is older server-only free-trial work in the legacy `slopus/agentrejoin-server` repository on branch:
