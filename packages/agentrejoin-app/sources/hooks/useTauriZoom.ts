...
const WEB_ZOOM_CLASS = 'agentrejoin-app-zoomed';
...
export function useTauriZoom() {
    useEffect(() => {
        ...
        if (!inTauri) {
            root.style.setProperty('--agentrejoin-app-zoom', getBrowserAppZoomValue());
            root.classList.add(WEB_ZOOM_CLASS);
            return () => {
                root.classList.remove(WEB_ZOOM_CLASS);
                root.style.removeProperty('--agentrejoin-app-zoom');
            };
        }
