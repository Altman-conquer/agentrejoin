...
const SERVER_PACKAGE_NAME = 'agentrejoin-server-self-host';
const SETTINGS_WRITE_CONFIRM_FLAG = '--i-understand-this-will-modify-default-agentrejoin-settings';
...
export async function handleServerCommand(args: string[]): Promise<void> {
    ...
    const dataDir = path.join(configuration.agentRejoinHomeDir, 'server-data');
    ...
    if (!artifacts) {
        console.error(chalk.red('Could not locate agentrejoin-server.'));
        ...
        console.error(chalk.gray('    - sibling packages/agentrejoin-server/sources/standalone.ts in the monorepo'));
    ...
    const env: NodeJS.ProcessEnv = {
        ...
        AGENTREJOIN_MASTER_SECRET: masterSecret,
        PORT: String(opts.port),
        HOST: opts.host,
    };
    if (staticDir) env.AGENTREJOIN_STATIC_DIR = staticDir;
    env.AGENTREJOIN_INJECT_HTML_CONFIG = JSON.stringify({
        serverUrl,
    ...
    if (artifacts.bundled) {
        ...
        if (!prismaEngine) {
            ...
            if (artifacts.source === 'package') {
                ...
                console.error(chalk.gray(`  Try reinstalling ${SERVER_PACKAGE_NAME}, then run \`agentrejoin server\` again.`));
            } else {
                console.error(chalk.gray('  Expected @prisma/engines to be available near the agentrejoin package.'));
                console.error(chalk.gray('  Try reinstalling agentrejoin, then run `agentrejoin server` again.'));
            }
            process.exit(1);
        }
    ...
    console.log();
    console.log(chalk.green.bold(`✓ agentrejoin-server starting at ${serverUrl}`));
    if (staticDir) {
        console.log(chalk.green(`  Open ${serverUrl} in your browser.`));
    }
    if (opts.persistServerUrl) {
        console.log(chalk.gray('  agentrejoin CLI + daemon will use this server automatically (settings.serverUrl).'));
    }
    ...
    const exitCode = await new Promise<number>(resolve => {
        child.on('exit', code => {
            console.log(chalk.gray(`\nagentrejoin-server exited (code ${code ?? 0})`));
            resolve(code ?? 0);
        });
    });
...
function showHelp() {
    console.log(`
${chalk.bold('agentrejoin server')} - Run AgentRejoin relay + web app locally (self-host)

${chalk.bold('Usage:')}
  agentrejoin server [--port 3005] [--host 127.0.0.1] [--reset] [--no-persist]
...
${chalk.bold('Notes:')}
  - Stores data in ${chalk.cyan('$AGENTREJOIN_HOME_DIR/server-data/')}
...
async function ensureSettingsWriteAllowed(opts: ServerOptions, serverUrl: string): Promise<void> {
    ...
    const message =
        `agentrejoin server will write settings.serverUrl and settings.webappUrl to ${serverUrl} ` +
 ...
 *
 * pkgroll bundles into dist/, so __dirname at runtime is .../agentrejoin-cli/dist; tools/ lives
 * at .../agentrejoin-cli/tools. In rare layouts (e.g. running un-built source via tsx from src/
 * commands/), tools/ sits at .../agentrejoin-cli/tools and __dirname is .../agentrejoin-cli/src/commands,
...
function bundledBinaryName(): string {
    return process.platform === 'win32' ? 'agentrejoin-server.exe' : 'agentrejoin-server';
}
...
/**
 * Resolves the artifacts needed to spawn agentrejoin-server.
 *
 * Order:
 *   1. agentrejoin-server-self-host package (npm-installed local server artifact)
 *   2. Legacy bundled binary at tools/server/<platform>/agentrejoin-server
 *   3. Source-mode fallback for monorepo dev: ../agentrejoin-server/sources/standalone.ts via tsx
 */
...
function findSourceStandalone(): string | undefined {
    const candidates = [
        path.resolve(__dirname, '../../../agentrejoin-server/sources/standalone.ts'),
        path.resolve(__dirname, '../../agentrejoin-server/sources/standalone.ts'),
        path.resolve(process.cwd(), 'packages/agentrejoin-server/sources/standalone.ts'),
        path.resolve(process.cwd(), '../agentrejoin-server/sources/standalone.ts'),
    ];
...
function findWebappDir(): string | undefined {
    ...
    const candidates = [
        path.resolve(__dirname, '../../../agentrejoin-app/dist'),
        path.resolve(__dirname, '../../agentrejoin-app/dist'),
        path.resolve(process.cwd(), 'packages/agentrejoin-app/dist'),
    ];
    for (const c of candidates) {
...
async function spawnAndWait(art: ServerArtifacts, env: NodeJS.ProcessEnv, args: string[]): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        child.on('exit', code => {
            if (code === 0) resolve();
            else reject(new Error(`agentrejoin-server ${args[0]} exited with code ${code}`));
        });
    });
}
