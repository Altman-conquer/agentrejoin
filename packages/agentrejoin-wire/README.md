...
Canonical wire specification package for AgentRejoin clients and services.
...
`agentrejoin-wire` centralizes definitions for:
...
In this repository, consumer workspaces import `agentrejoin-wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace agentrejoin-wire build`
...
yarn workspace agentrejoin-wire build
yarn workspace agentrejoin-wire test
```
