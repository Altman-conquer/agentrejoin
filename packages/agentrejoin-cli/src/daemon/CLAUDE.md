# AgentRejoin CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages AgentRejoin sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `agentrejoin daemon start`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-agentrejoin-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or AGENTREJOIN_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade agentrejoin` occurs:
...
Command: `agentrejoin daemon stop`
...
1. Backend forwards RPC `spawn-agentrejoin-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached AgentRejoin process with `--agentrejoin-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New AgentRejoin process:
   - Creates session with backend, receives `agentRejoinSessionId`
   - Calls `notifyDaemonSessionStarted()` to POST to daemon's `/session-started`
5. Daemon updates tracking with `agentRejoinSessionId`, resolves awaiter
6. RPC returns session info to mobile app

### Terminal-Spawned Sessions

User runs `agentrejoin` directly:
1. CLI auto-starts daemon if configured
2. AgentRejoin process calls `notifyDaemonSessionStarted()`
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'agentrejoin directly...'`
4. Session tracked for health monitoring

### Session Termination

Via RPC `stop-session` or health check:
1. `stopSession()` finds session by `agentRejoinSessionId`
...
`agentrejoin doctor` uses `ps aux | grep` to find all AgentRejoin processes:
- Production: matches `agentrejoin.mjs`, `agentrejoin`, or `dist/index.mjs`
...
- Server to Daemon: rpc-request (spawn-agentrejoin-session, stop-session, requestShutdown)
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'platform': 'darwin',
    'agentRejoinCliVersion': '1.0.0',
    'homeDir': '/Users/john',
    'agentRejoinHomeDir': '/Users/john/.agentrejoin'
  }))",
...
socket.emit('machine-update-metadata', {
  ...
  "metadata": "base64(encrypted({
    'host': 'MacBook-Pro.local',
    'platform': 'darwin',
    'agentRejoinCliVersion': '1.0.1',  // version updated
    'homeDir': '/Users/john',
    'agentRejoinHomeDir': '/Users/john/.agentrejoin'
  }))",
  "expectedVersion": 1
