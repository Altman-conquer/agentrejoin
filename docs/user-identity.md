...
How a single AgentRejoin user is identified across every external service.

## Primary ID: AgentRejoin Account CUID
...
```
AgentRejoin Account CUID (e.g. cm4x7k2...)
│
...
POST /v1/auth { publicKey, challenge, signature }
  ...
  ├─ auth.createToken(CUID)  →  JWT (signed with AGENTREJOIN_MASTER_SECRET)
  │
  ▼
...
| System | ID Type | Why |
|--------|---------|-----|
| ElevenLabs | HMAC-derived | Privacy — raw AgentRejoin ID never sent to ElevenLabs |
...
To derive an ElevenLabs user ID from a AgentRejoin CUID locally:
