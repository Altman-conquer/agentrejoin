...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.AGENTREJOIN_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.AGENTREJOIN_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.AGENTREJOIN_HOME_DIR = join(env.envDir, 'cli', 'home');
    process.env.AGENTREJOIN_PROJECT_DIR = env.projectPath;
    process.env.AGENTREJOIN_VARIANT = 'dev';
    process.env.DEBUG = '1';
}
