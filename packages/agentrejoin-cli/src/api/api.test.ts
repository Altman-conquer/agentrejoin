...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when AgentRejoin server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  AgentRejoin server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when AgentRejoin server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when AgentRejoin server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
