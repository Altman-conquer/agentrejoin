 ...
 * - connect codex: Store OpenAI API key in AgentRejoin cloud
 * - connect claude: Store Anthropic API key in AgentRejoin cloud
 * - connect gemini: Store Gemini API key in AgentRejoin cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('agentrejoin connect')} - Connect AI vendor API keys to AgentRejoin

${chalk.bold('Usage:')}
  agentrejoin connect codex        Store your Codex API key in AgentRejoin
  agentrejoin connect claude       Store your Anthropic API key in AgentRejoin
  agentrejoin connect gemini       Store your Gemini API key in AgentRejoin
  agentrejoin connect status       Show connection status for all vendors
  agentrejoin connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in AgentRejoin. This enables you to use these services through AgentRejoin
...
${chalk.bold('Notes:')} 
  • You must be authenticated with AgentRejoin first (run 'agentrejoin auth login')
  • API keys are encrypted and stored securely by your configured relay
  • You can manage stored keys in the connected AgentRejoin web app
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to AgentRejoin\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with AgentRejoin'));
        console.log(chalk.gray('  Please run "agentrejoin auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: agentrejoin connect <vendor>'));
    console.log(chalk.gray('Example: agentrejoin connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with AgentRejoin cloud
 * This ensures the Gemini SDK uses the same account as AgentRejoin
 */
