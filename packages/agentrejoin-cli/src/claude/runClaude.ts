...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --agentrejoin-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    const forkedFromSessionId = process.env.AGENTREJOIN_FORKED_FROM_SESSION_ID;
    const forkedFromMessageId = process.env.AGENTREJOIN_FORKED_FROM_MESSAGE_ID;
    const isSideChat = process.env.AGENTREJOIN_SIDE_CHAT === '1';
    ...
    let metadata: Metadata = {
        ...
        agentRejoinToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    const reconnectSessionId = process.env.AGENTREJOIN_RECONNECT_SESSION_ID;
    const reconnectKeyBase64 = process.env.AGENTREJOIN_RECONNECT_ENCRYPTION_KEY;
    const reconnectVariant = process.env.AGENTREJOIN_RECONNECT_ENCRYPTION_VARIANT as 'legacy' | 'dataKey' | undefined;
    const reconnectSeq = process.env.AGENTREJOIN_RECONNECT_SEQ;
    const reconnectMetadataVersion = process.env.AGENTREJOIN_RECONNECT_METADATA_VERSION;
    const reconnectAgentStateVersion = process.env.AGENTREJOIN_RECONNECT_AGENT_STATE_VERSION;
    ...
    // Fork backfill: when this AgentRejoin session was just spawned as a fork
    // of another (AGENTREJOIN_FORK_CLAUDE_SESSION_ID is set by the daemon at
    ...
    // historical messages back to the AgentRejoin client — so without an
    ...
    //
    // Skipped on reconnect (AGENTREJOIN_RECONNECT_*) — that path reattaches
    // to the existing AgentRejoin session, where the server already has every
    // message it needs.
    const forkClaudeSessionId = process.env.AGENTREJOIN_FORK_CLAUDE_SESSION_ID;
    if (!reconnectSessionId && forkClaudeSessionId) {
        ...
        // Bind the new AgentRejoin session to the forked Claude UUID up front so the
    ...
    // alongside this AgentRejoin session. App-sent prompts also land in the
    ...
    // to the running AgentRejoin session. SDK-emitted assistant + tool_result
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: happyServer.toolNames.map(toolName => `mcp__agentrejoin__${toolName}`),
