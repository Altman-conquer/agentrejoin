...
Goal: make `agentrejoin-agent` the reliable control plane for dispatching and monitoring the rest of this roadmap.

### Required outcomes

- Verify the current `agentrejoin-agent` implementation on the real stack from this current environment before using it to spawn work for the rest of the roadmap.
- Fix any blocking issues in the current branch first, rather than assuming `agentrejoin-agent` is ready and immediately branching into many worktrees.
...
- Use `agentrejoin-agent` to create worktrees and spawn new agent sessions only after the base flow is proven locally.
...
- Finish and validate `agentrejoin-agent spawn`, mirroring the app's `spawn-agentrejoin-session` flow.
...
- Only after this passes should the other roadmap items be delegated through `agentrejoin-agent`.
...
The session protocol (`role: 'session'` envelopes in `agentrejoin-wire/src/sessionProtocol.ts`) is **not used in production** and should not be used in dev environments either until we revisit the design. The legacy protocol (`role: 'user'` / `role: 'agent'`) is the active code path everywhere.

### Status

- Types are frozen in `agentrejoin-wire` — no new consumers
...
- Consider whether `agentrejoin-wire` should even own this, or if protocol definition belongs closer to the CLI / agent layer
...
- Growth tracks:
  ...
  - software factory / `agentrejoin-agent`
...
- Native app test flow:
  ...
  3. For JS-only work, start Metro without recompiling native: `APP_ENV=development yarn --cwd packages/agentrejoin-app start --dev-client --port 8081`.
