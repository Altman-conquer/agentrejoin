...
function redisStats() {
    try {
        const streamLen = execSync(`kubectl exec agentrejoin-redis-0 -c redis -- redis-cli xlen socket.io 2>/dev/null`).toString().trim();
        const info = execSync(`kubectl exec agentrejoin-redis-0 -c redis -- redis-cli info all 2>/dev/null`).toString();
