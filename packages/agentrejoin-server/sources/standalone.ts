...
export async function runMigrations(opts: { pgliteDir: string; migrationsDir?: string } = { pgliteDir }) {
    ...
    candidates.push(
        ...
        path.join(process.cwd(), "packages", "agentrejoin-server", "prisma", "migrations"),
...
async function serve() {
    ...
    const masterSecret = process.env.AGENTREJOIN_MASTER_SECRET;
    if (!masterSecret) {
        throw new Error("AGENTREJOIN_MASTER_SECRET is required");
    }
    ...
    if (process.env.AGENTREJOIN_INJECT_HTML_CONFIG) {
        try {
            injectHtmlConfig = JSON.parse(process.env.AGENTREJOIN_INJECT_HTML_CONFIG);
        } catch {
...
function findStaticDir(): string | undefined {
    const candidates = [
        process.env.AGENTREJOIN_STATIC_DIR,
...
export function isStandaloneEntrypoint(invokedFile: string): boolean {
    ...
    // agentrejoin-server.exe when tests or tooling run cross-platform.
...
if (isDirectInvocation) {
    ...
    switch (command) {
        ...
        case "serve":
            ...
            break;
        default:
            console.log(`agentrejoin-server - portable distribution
...
Environment variables:
  ...
  AGENTREJOIN_MASTER_SECRET  Required: master secret for auth/encryption
`);
