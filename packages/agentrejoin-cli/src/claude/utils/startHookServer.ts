 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.agentrejoin/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `agentrejoin` (fresh start) - new session created
 * - `agentrejoin --continue` - continues last session (may fork)
 * - `agentrejoin --resume` - interactive picker, then resume
 * - `agentrejoin --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple AgentRejoin processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between AgentRejoin process and Claude session.
 */
