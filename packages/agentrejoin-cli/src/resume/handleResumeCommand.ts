...
export function parseResumeCommandArgs(args: string[]): { showHelp: boolean; sessionId: string } {
    ...
    if (args.length === 0) {
        throw new Error('AgentRejoin session ID is required: agentrejoin resume <session-id>');
    }
    if (args.length > 1) {
        throw new Error(`Unexpected arguments for agentrejoin resume: ${args.slice(1).join(' ')}`);
    }
...
export function buildResumeLaunch(session: ResumableHappySession, options: ResumeLaunchOptions = {}): ResumeLaunch {
    ...
    if (flavor === 'claude') {
        ...
        if (options.claudeStartingMode) {
            args.push('--agentrejoin-starting-mode', options.claudeStartingMode);
        }
        if (options.startedBy) {
            args.push('--started-by', options.startedBy);
        }
...
export function formatResumeHelp(): string {
    return [
        'agentrejoin resume - Resume a previous AgentRejoin session',
        '',
        'Usage:',
        '  agentrejoin resume <agentrejoin-session-id>',
        '',
        'Examples:',
        '  agentrejoin resume cmmij8olq00dp5jcxr3wtbpau',
        '  agentrejoin resume cmmij8',
        '',
...
function buildReconnectEnv(session: ReconnectableHappySession): NodeJS.ProcessEnv {
    return buildSessionChildEnvironment(process.env, {
        AGENTREJOIN_RECONNECT_SESSION_ID: session.id,
        AGENTREJOIN_RECONNECT_ENCRYPTION_KEY: encodeBase64(session.encryptionKey),
        AGENTREJOIN_RECONNECT_ENCRYPTION_VARIANT: session.encryptionVariant,
        AGENTREJOIN_RECONNECT_SEQ: String(session.seq),
        AGENTREJOIN_RECONNECT_METADATA_VERSION: String(session.metadataVersion),
        AGENTREJOIN_RECONNECT_AGENT_STATE_VERSION: String(session.agentStateVersion),
    });
}
