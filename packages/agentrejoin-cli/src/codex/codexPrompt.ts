...
export interface CodexEnhancedMode {
    ...
    model?: string;
    /** AgentRejoin app instructions appended to the first Codex prompt for option chips. */
...
/**
 * AgentRejoin wraps its own injected instructions (the option-chips system prompt and
 ...
 */
export const AGENTREJOIN_SYSTEM_BLOCK_OPEN = '<agentrejoin-system>';
export const AGENTREJOIN_SYSTEM_BLOCK_CLOSE = '</agentrejoin-system>';

function wrapHappySystem(text: string): string {
    return `${AGENTREJOIN_SYSTEM_BLOCK_OPEN}\n${text}\n${AGENTREJOIN_SYSTEM_BLOCK_CLOSE}`;
}

/**
 * Remove any `<agentrejoin-system>…</agentrejoin-system>` blocks (and the blank lines that
...
export function stripHappySystemBlocks(text: string): string {
    const open = AGENTREJOIN_SYSTEM_BLOCK_OPEN.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const close = AGENTREJOIN_SYSTEM_BLOCK_CLOSE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
