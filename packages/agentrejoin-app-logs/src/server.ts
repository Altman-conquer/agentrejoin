...
// Resolve agentrejoin home dir — same pattern as agentrejoin-cli/src/configuration.ts
let happyHome: string;
if (process.env.AGENTREJOIN_HOME_DIR) {
    happyHome = process.env.AGENTREJOIN_HOME_DIR.replace(/^~/, homedir());
} else {
    happyHome = join(homedir(), '.agentrejoin');
}
