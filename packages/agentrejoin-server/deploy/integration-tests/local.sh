...
echo "=== AgentRejoin Local Deployment (minikube) ==="
...
echo "Building agentrejoin-server:local image..."
docker build -t agentrejoin-server:local -f "$REPO_ROOT/Dockerfile.server" "$REPO_ROOT"
...
kubectl wait --for=condition=ready pod -l app=agentrejoin-postgres --timeout=60s

# Run migrations via a one-shot job
kubectl run agentrejoin-migrate --rm -i --restart=Never \
    --image=agentrejoin-server:local \
    --image-pull-policy=Never \
    --env="DATABASE_URL=postgresql://agentrejoin:agentrejoin@agentrejoin-postgres:5432/agentrejoin" \
    -- sh -c "cd /repo && npx prisma migrate deploy --schema=packages/agentrejoin-server/prisma/schema.prisma" \
    2>/dev/null || true
