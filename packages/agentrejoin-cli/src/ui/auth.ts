...
export async function doAuth(): Promise<Credentials | null> {
    ...
    try {
        ...
        await axios.post(`${configuration.serverUrl}/v1/auth/request`, {
            ...
            headers: {
                'X-AgentRejoin-Client': `cli/${configuration.currentCliVersion}`
            }
        });
        if (process.env.DEBUG) {
...
async function doMobileAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
    ...
    console.log('Scan this QR code with your AgentRejoin app:\n');

    const authUrl = 'agentrejoin://terminal?' + encodeBase64Url(keypair.publicKey);
...
async function doWebAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
    ...
    // someone running agentrejoin inside a devcontainer that they saw the
    // "Complete authentication in your browser window." but nothing opened.
    // https://github.com/slopus/agentrejoin/issues/19
