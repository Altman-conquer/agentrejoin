...
import { createEnvelope, type CreateEnvelopeOptions, type SessionEnvelope, type SessionTurnEndStatus } from 'agentrejoin-wire';
import {
...
function buildMultipartUploadBody(
    ...
    const boundary = `----agentrejoin-cli-${randomUUID()}`;
    const chunks: Buffer[] = [];
...
export class ApiSessionClient extends EventEmitter {
    ...
    constructor(token: string, session: Session) {
        ...
        this.socket = io(configuration.serverUrl, {
            auth: {
                ...
                agentRejoinClient: `cli-coding-session/${configuration.currentCliVersion}`
            },
            path: '/v1/updates',
            reconnection: false,
     ...
     * Legacy sessions use deriveKey(masterSecret, 'AgentRejoin Blobs', ['master']).
     * DataKey sessions use deriveKey(dataKey, 'AgentRejoin Blobs', ['session']).
     */
    async getBlobKey(): Promise<Uint8Array> {
        if (!this.blobKey) {
            const path = this.encryptionVariant === 'dataKey' ? ['session'] : ['master'];
            this.blobKey = await deriveKey(this.encryptionKey, 'AgentRejoin Blobs', path);
        }
        return this.blobKey;
    }
    ...
    private authHeaders() {
        return {
            ...
            'X-AgentRejoin-Client': `cli-coding-session/${configuration.currentCliVersion}`
        };
    }
