 ...
 *
 * This file boots one authenticated AgentRejoin environment and restarts the daemon
...
async function stopAllTrackedSessions(): Promise<void> {
  ...
  await Promise.all(
    sessions.map((session: any) =>
      stopDaemonSession(session.agentRejoinSessionId ?? `PID-${session.pid}`).catch(() => false)
    ),
  );
}
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      agentRejoinHomeDir: '/test/agentrejoin-home',
      agentRejoinLibDir: '/test/agentrejoin-lib',
      agentRejoinToolsDir: '/test/agentrejoin-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('agentrejoin directly - likely by user from terminal');
    expect(tracked.agentRejoinSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.agentRejoinSessionId === response.sessionId
    );
    ...
    expect(spawnedSession.agentRejoinSessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.agentRejoinSessionId);
  });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real agentrejoin process that looks like it was started from terminal
    ...
    if (!terminalHappyProcess || !terminalHappyProcess.pid) {
      throw new Error('Failed to spawn terminal agentrejoin process');
    }
    ...
    const daemonSession = sessions.find(
      (s: any) => s.agentRejoinSessionId === spawnResponse.sessionId
    );

    expect(terminalSession).toBeDefined();
    expect(terminalSession.startedBy).toBe('agentrejoin directly - likely by user from terminal');
    ...
    await stopDaemonSession(daemonSession.agentRejoinSessionId);
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.agentRejoinSessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.agentRejoinSessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.agentRejoinSessionId).toBeDefined();
      await stopDaemonSession(session.agentRejoinSessionId);
    }
  });
   ...
   * 
   * This simulates what happens during `npm upgrade agentrejoin`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/agentrejoin/ with NEW version files
  ...
  it.skip('[skipped] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.AGENTREJOIN_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when agentrejoin is uninstalled
