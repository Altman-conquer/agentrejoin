...
// Suffix host with `-dev` for the AGENTREJOIN_VARIANT=dev variant so the dev daemon
// is visually distinct from the stable one in the machine list (they otherwise
// share the same hostname and look identical).
const hostSuffix = process.env.AGENTREJOIN_VARIANT === 'dev' ? '-dev' : '';
...
export async function startDaemon(): Promise<void> {
  ...
  let requestShutdown: (source: 'agentrejoin-app' | 'agentrejoin-cli' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'agentrejoin-app' | 'agentrejoin-cli' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  try {
    ...
    // Handle webhook from agentrejoin session reporting itself
    const onHappySessionWebhook = (sessionId: string, sessionMetadata: Metadata, encryption?: SessionEncryptionData) => {
      ...
      if (existingSession && existingSession.startedBy === 'daemon') {
        ...
        const trackedSession: TrackedSession = {
          startedBy: 'agentrejoin directly - likely by user from terminal',
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (options.parentSessionId) {
          extraEnv.AGENTREJOIN_FORKED_FROM_SESSION_ID = options.parentSessionId;
        }
        if (options.forkedFromMessageId) {
          extraEnv.AGENTREJOIN_FORKED_FROM_MESSAGE_ID = options.forkedFromMessageId;
        }
        if (options.isSideChat) {
          extraEnv.AGENTREJOIN_SIDE_CHAT = '1';
        }
        // For fork: spawned AgentRejoin CLI needs to know which Claude JSONL to
        // backfill into the fresh AgentRejoin session row. Without this, the
        ...
        if (options.resumeClaudeSessionId) {
          extraEnv.AGENTREJOIN_FORK_CLAUDE_SESSION_ID = options.resumeClaudeSessionId;
        }
        if (options.resumeCodexThreadId) {
          extraEnv.AGENTREJOIN_FORK_CODEX_THREAD_ID = options.resumeCodexThreadId;
        }
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          const resumeId = agent === 'claude'
            ...
            : (agent === 'codex'
              ? options.resumeCodexThreadId
              : (agent === 'gemini' ? options.resumeGeminiSessionId : undefined));
          const resumeFragment = resumeId
            ? ` --resume ${shellescape(resumeId)}`
            : '';
          const launchArgs = [
            agent,
            '--agentrejoin-starting-mode', 'remote',
            '--started-by', 'daemon',
          ];
          ...
          const windowName = `agentrejoin-${Date.now()}-${agent}`;
          ...
          if (tmuxResult.success) {
            ...
            // Wait for webhook to populate session with agentRejoinSessionId (exact same as regular flow)
            ...
            return new Promise((resolve) => {
              ...
              pidToAwaiter.set(tmuxResult.pid!, (completedSession) => {
                ...
                logger.debug(`[DAEMON RUN] Session ${completedSession.agentRejoinSessionId} fully spawned with webhook (tmux)`);
                resolve({
                  type: 'success',
                  sessionId: completedSession.agentRejoinSessionId!
                });
              });
            });
          } else {
        ...
        if (!useTmux) {
          ...
          // Resume ids attach the new AgentRejoin session to a pre-existing provider
          ...
          if (options.resumeGeminiSessionId && agentCommand === 'gemini') {
            args.push('--resume', options.resumeGeminiSessionId);
          }
    ...
    const spawnTrackedHappyProcess = ({
      ...
      return new Promise((resolve) => {
        ...
        pidToAwaiter.set(happyProcess.pid!, (completedSession) => {
          clearTimeout(timeout);
          logger.debug(`[DAEMON RUN] Session ${completedSession.agentRejoinSessionId} fully spawned with webhook`);
          resolve({
            type: 'success',
            sessionId: completedSession.agentRejoinSessionId!
          });
    ...
    const resumeSession = async (agentRejoinSessionId: string, options?: { model?: string; permissionMode?: string }): Promise<SpawnSessionResult> => {
      try {
        ...
        if (!tracked) {
          return { type: 'error', errorMessage: `Session ${agentRejoinSessionId} is not tracked by this daemon. It may have been started before the daemon or on another machine.` };
        }
        if (!tracked.happySessionMetadataFromLocalWebhook) {
          return { type: 'error', errorMessage: `Session ${agentRejoinSessionId} has no metadata. Cannot resume.` };
        }
        ...
        const launch = buildResumeLaunch(
          { id: agentRejoinSessionId, active: true, metadata },
    ...
    const onChildExited = (pid: number) => {
      ...
      if (session?.agentRejoinSessionId && session.encryption) {
        sessionIdToFinishedSession.set(session.agentRejoinSessionId, session);
        logger.debug(`[DAEMON RUN] Process PID ${pid} exited, preserved session ${session.agentRejoinSessionId} for resume`);
      } else {
    ...
    const { port: controlPort, stop: stopControlServer } = await startDaemonControlServer({
      ...
      stopSession,
      spawnSession,
      requestShutdown: () => requestShutdown('agentrejoin-cli'),
    ...
    // when npm replaces `dist/index.mjs` on disk (= the user ran `npm i -g agentrejoin`).
    ...
    // diverged from the bundled version (e.g. `agentrejoin-coder@0.13.1` deprecation
    ...
    apiMachine.setRPCHandlers({
      ...
      resumeSession,
      stopSession,
      requestShutdown: () => requestShutdown('agentrejoin-app')
    });
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.AGENTREJOIN_DAEMON_HEARTBEAT_INTERVAL || '60000');
    ...
    const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
      ...
      if (bundleReplaced) {
        ...
        // `agentrejoin daemon start` reads our still-present daemon.state.json, sees
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'agentrejoin-app' | 'agentrejoin-cli' | 'os-signal' | 'exception', errorMessage?: string) => {
