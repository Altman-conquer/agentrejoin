...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.agentrejoin-servers.com') */
    serverUrl: string;
...
export function startOfflineReconnection<TSession>(
    ...
    const defaultHealthCheck = async () => {
        await axios.get(`${config.serverUrl}/v1/sessions`, {
            ...
            headers: {
                'X-AgentRejoin-Client': `cli-daemon/${configuration.currentCliVersion}`
            }
        });
    };
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `agentrejoin auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `agentrejoin auth`',
