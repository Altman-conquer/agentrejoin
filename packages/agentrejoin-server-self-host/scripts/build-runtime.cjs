 ...
 * around packages/agentrejoin-server, which stays private and is what production
 * deploys. We reach into the sibling at build time only (same pattern as
 * agentrejoin-cli/scripts/bundle-webapp.cjs reaching into agentrejoin-app).
 *
 * Produces:
 *   dist/standalone.mjs  — bun bundle of ../agentrejoin-server/sources/standalone.ts
...
const serverPackage = path.resolve(root, '..', 'agentrejoin-server');
