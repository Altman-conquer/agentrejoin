...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `agentrejoin sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('agentrejoin sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  agentrejoin sandbox configure      Configure sandbox settings interactively
  agentrejoin sandbox status         Show current sandbox configuration
  agentrejoin sandbox disable        Disable sandboxing
  agentrejoin sandbox help           Show this help
`);
}
