...
function getLatestStableTag() {
  const tagsRaw = execSync(`git tag --list "agentrejoin-agent-v*" --sort=-v:refname`, {
    encoding: "utf8",
  });
  ...
  // Only accept stable semver tags like agentrejoin-agent-v1.2.3 (no prerelease suffix).
  const stableTag = tags.find((t) => /^agentrejoin-agent-v\d+\.\d+\.\d+$/.test(t));
...
async function generateReleaseNotes() {
  try {
    ...
    try {
      gitLog = execSync(
        `git log ${commitRange} --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/agentrejoin-agent`,
        { encoding: "utf8" }
      );
    } catch (error) {
      console.error(
      ...
      gitLog = execSync(
        `git log -10 --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/agentrejoin-agent`,
        { encoding: "utf8" }
      );
    }
    ...
    const prompt = `Please analyze these git commits and generate professional release notes for version ${toVersion} of the AgentRejoin control CLI tool (a remote agent control CLI).

The release should cover commits since the latest stable agentrejoin-agent tag (agentrejoin-agent-vX.Y.Z): ${
      fromTag ?? "(none)"
    }.
