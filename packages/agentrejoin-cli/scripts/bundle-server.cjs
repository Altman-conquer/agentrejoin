...
/**
 * Bundles agentrejoin-server into a self-contained artifact directory.
 *
 * Uses `bun build --compile` to produce a single platform-specific binary, then copies the
 * pglite WASM/data files and prisma migrations alongside. agentrejoin-cli does NOT depend on the
 * agentrejoin-server workspace package — we reach into the sibling directory at build time only.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
 *       agentrejoin-server                  # bun-compiled binary
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/agentrejoin-server');
...
function platformBinaryName(plat) {
    return plat.endsWith('-win32') ? 'agentrejoin-server.exe' : 'agentrejoin-server';
}
...
function buildPlatform(plat, outRoot) {
    ...
    console.log(`\n→ Bundling agentrejoin-server for ${plat} (${target})`);
    run(
        'bun',
        [
            'build',
...
function main() {
    ...
    console.log(`\n✓ agentrejoin-server bundle written to ${outDir}`);
}
