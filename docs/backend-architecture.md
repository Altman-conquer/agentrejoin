...
This document describes the AgentRejoin backend structure as implemented in `packages/agentrejoin-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/agentrejoin-server/sources/main.ts`.
...
3. Initialize crypto modules:
   - `initEncrypt()` derives a KeyTree from `AGENTREJOIN_MASTER_SECRET`.
...
- Tokens are generated and verified by privacy-kit using `AGENTREJOIN_MASTER_SECRET`.
...
graph TB
    ...
    S1 & S2 & S3 & S4 --> |KeyTree from AGENTREJOIN_MASTER_SECRET| DB
...
- The server only encrypts/decrypts **service tokens** (GitHub OAuth tokens, vendor tokens) using the KeyTree derived from `AGENTREJOIN_MASTER_SECRET`.
...
- Entrypoint: `packages/agentrejoin-server/sources/main.ts`
- API server: `packages/agentrejoin-server/sources/app/api/api.ts`
- Socket server: `packages/agentrejoin-server/sources/app/api/socket.ts`
- Event routing: `packages/agentrejoin-server/sources/app/events/eventRouter.ts`
- Presence: `packages/agentrejoin-server/sources/app/presence`
- Storage: `packages/agentrejoin-server/sources/storage`
- Prisma schema: `packages/agentrejoin-server/prisma/schema.prisma`
