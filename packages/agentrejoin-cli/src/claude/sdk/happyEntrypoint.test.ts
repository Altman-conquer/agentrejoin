...
import { AGENTREJOIN_DEFAULT_ENTRYPOINT, resolveHappyEntrypoint } from './happyEntrypoint'

describe('resolveHappyEntrypoint', () => {
    it('falls back to the AgentRejoin default when no value is set', () => {
        expect(resolveHappyEntrypoint(undefined)).toBe(AGENTREJOIN_DEFAULT_ENTRYPOINT)
        expect(resolveHappyEntrypoint('')).toBe(AGENTREJOIN_DEFAULT_ENTRYPOINT)
    })
    ...
    it('uses a default that is NOT in the SDK picker-filter set', () => {
        ...
        // entrypoint is one of these. The AgentRejoin default must avoid that set
        // so AgentRejoin-touched sessions stay visible in the picker.
        const sdkPickerFilterSet = new Set(['sdk-cli', 'sdk-ts', 'sdk-py'])
        expect(sdkPickerFilterSet.has(AGENTREJOIN_DEFAULT_ENTRYPOINT)).toBe(false)
    })
    ...
    it('uses a default Claude Code recognises as a valid entrypoint', () => {
        ...
        expect(claudeKnownEntrypoints.has(AGENTREJOIN_DEFAULT_ENTRYPOINT)).toBe(true)
    })
})
