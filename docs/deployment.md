...
This document describes how to deploy the AgentRejoin backend (`packages/agentrejoin-server`) and the infrastructure it expects.
...
2. **Redis**
   ...
   - Managed by this repo: `packages/agentrejoin-server/deploy/agentrejoin-redis.yaml` (StatefulSet + redis-exporter sidecar).
...
- `AGENTREJOIN_MASTER_SECRET`: master key for auth tokens and server-side encryption.
...
Example manifests live in `packages/agentrejoin-server/deploy`:
- `handy.yaml`: Deployment + Service + ExternalSecrets for the server.
- `agentrejoin-redis.yaml`: Redis StatefulSet + Service + ConfigMap.
...
- `pnpm --filter agentrejoin-server db` (Postgres in Docker)
- `pnpm --filter agentrejoin-server redis`
- `pnpm --filter agentrejoin-server s3` + `s3:init`

Use `.env`/`.env.dev` to load local settings when running `pnpm --filter agentrejoin-server dev`.

## Implementation references
- Entrypoint: `packages/agentrejoin-server/sources/main.ts`
- Dockerfile: `Dockerfile.server`
- Kubernetes manifests: `packages/agentrejoin-server/deploy`
- Env usage: `packages/agentrejoin-server/sources` (`rg -n "process.env"`)
