...
register.setDefaultLabels({ app: 'agentrejoin-server' });
...
export function getMetricsLabelsFromSocket(socket: Socket): ClientLabels {
    return parseClientLabels(socket.data.agentRejoinClient as string);
}
...
export function getMetricsLabelsFromRequest(request: { headers: Record<string, string | string[] | undefined> }): ClientLabels {
    return parseClientLabels(request.headers['x-agentrejoin-client'] as string);
}
