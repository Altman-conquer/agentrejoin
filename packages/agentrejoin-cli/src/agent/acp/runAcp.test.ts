...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockProjectPath: vi.fn(() => '/tmp/agentrejoin'),
...
vi.mock('@/daemon/run', () => ({
  initialMachineMetadata: { host: 'host', platform: 'darwin', agentRejoinCliVersion: 'test', homeDir: '/tmp', agentRejoinHomeDir: '/tmp/.agentrejoin', agentRejoinLibDir: '/tmp/agentrejoin' },
}));
...
describe('runAcp', () => {
  ...
  it('wires backend messages through mapper into session envelopes', async () => {
    ...
    expect(consoleLines()).toEqual(expect.arrayContaining([
      'AgentRejoin Session ID: session-1',
  ...
  it('updates session metadata with ACP config options (models and operating modes)', async () => {
    ...
    const baseMetadata = {
      ...
      agentRejoinLibDir: '/repo/.agentrejoin/lib',
      agentRejoinToolsDir: '/repo/.agentrejoin/tools',
    };
