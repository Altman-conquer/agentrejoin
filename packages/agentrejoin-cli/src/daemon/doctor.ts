...
/**
 * Find all AgentRejoin CLI processes (including current process)
 */
export async function findAllHappyProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Check if it's a AgentRejoin process
      const isHappy = name.includes('agentrejoin') ||
                      name === 'node' && (cmd.includes('agentrejoin-cli') || cmd.includes('dist/index.mjs')) ||
                      cmd.includes('agentrejoin.mjs') ||
                      cmd.includes('agentrejoin-coder') || // legacy npm package name
                      cmd.includes('/agentrejoin/') ||
                      (cmd.includes('tsx') && cmd.includes('src/index.ts') && cmd.includes('agentrejoin-cli'));
...
/**
 * Find all runaway AgentRejoin CLI processes that should be killed
 */
...
/**
 * Kill all runaway AgentRejoin CLI processes
 */
